...
# 2. Load data
###############################
x_caliculate <- read.csv("x_virus_caliculate.csv", header = F)
...
ggsave(file = "x_virus_caliculate_plot.png", plot = g)

x_caliculate <- read.csv("x_virus_caliculate_a.csv", header = F)
names(x_caliculate) <- c("t", "x", "y")
x_caliculate <- as.data.frame(x_caliculate)
x_caliculate <- x_caliculate[1:1001,1:3]
